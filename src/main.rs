...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mod_dirs = generate_msgs(&target, &arg_path)?;
    generate_lib_rs_and_toml(&mod_dirs)?;

    // generate
    // target/Cargo.toml
    // target/src/lib.rs
    generate_top_toml_lib_rs(&target, project_name.to_str().unwrap(), &mod_dirs)?;

    Ok(())
}

fn generate_top_toml_lib_rs(
    target: &PathBuf,
    project_name: &str,
    ...
    for (dir, _) in mod_dirs {
        let file_name = dir.file_name().unwrap().to_str().unwrap();
        deps = format!("{deps}{file_name} = {{ path = \"{file_name}\" }}\n");
    }

    let toml = format!(
        r#"# This file was automatically generated by ros2msg_to_rs (https://github.com/tier4/ros2msg_to_rs).
[package]
name = "{project_name}"
version = "0.1.0"
edition = "2021"
...
{deps}"#
    ...
    let toml_file = target.join("Cargo.toml");
    println!("generating: {}", toml_file.display());
    let mut f = File::create(toml_file)?;
    ...
    for (dir, _) in mod_dirs {
        let file_name = dir.file_name().unwrap().to_str().unwrap();
        exports = format!("{exports}pub use {file_name};\n");
    }
    ...
    println!("generating: {}", lib_rs.display());
    let mut f = File::create(lib_rs)?;
    f.write(exports.as_bytes())?;

    Ok(())
}

fn generate_lib_rs_and_toml(
    mod_dirs: &BTreeMap<PathBuf, BTreeSet<String>>,
) -> Result<(), Box<dyn Error>> {
    for (dir, libs) in mod_dirs {
        // generating src/lib.rs
        let lib_rs = dir.join("src").join("lib.rs");
        println!("generating: {}", lib_rs.display());
        let mut lib_rs = File::create(lib_rs)?;
        lib_rs.write(b"// This file was automatically generated by ros2msg_to_rs (https://github.com/tier4/ros2msg_to_rs).\n#[allow(unused_imports)]\npub mod msg;")?;
        ...
        let mut deps = String::new();
        for (dir2, _) in mod_dirs {
            let lib = dir2.file_name().unwrap().to_str().unwrap();

            if libs.contains(lib) {
                deps = format!("{deps}{lib} = {{ path = \"../{lib}\" }}\n");
            }
        }
                ...
                if ext == "msg" {
                    if let Some(type_name) = p.file_name() {
                        ...
                        match parser::parse_msg(&contents).finish() {
                            Ok((_, result)) => {
                                // generate Rust code
                                let mut g = Generator::new(mod_name.to_str().unwrap().to_string());
                                let lines =
                                ...
                                // "target/{mod_name}/src"
                                let crate_dir = mod_dir.join("src");
...
";
    ...
    fn generate(input: &str) {
        let mut g = Generator::new("my_library".to_string());
